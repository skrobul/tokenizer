...
exports.tokenize = function(txt, extractors) {
  ...
  // sort by indexes now
  extracted_fields = sortKey(extracted_fields, 'index')
  ...
  for(i=0; i < extracted_fields.length; i++) {
    ...
    if(curr_pos < next_field.index) {
      ...
      out_arr.push({ type: 'text', txt: rawstr_to_add, len: rawstr_to_add.length, index: curr_pos })
  ...
  if (curr_pos < txt.length) {
    ...
    out_arr.push({ type: 'text', txt: remaining_text, len: remaining_text.length, index: curr_pos })
  }
